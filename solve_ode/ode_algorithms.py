...
# from forces_accelerations.forces import compute_forces # Cambiar con nombre real
# from output.save_text_file import to_txt # Cambiar por nombre real

# Calcular energias en el de alain

def velocity_verlet(positions, velocities, accelerations, n_at, delta_t, cutoff):
    ...
    if max(displacements) >= cutoff:
        accelerations = forces(positions, recalculate = True)
    else:
        accelerations = forces(positions)
        
    velocities = inter_velocities + 0.5 * accelerations * delta_t
    
    to_txt(positions, velocities, accelerations) # Save in a text file the new coordinates

    return positions, velocities, accelerations

def leap_frog(positions, velocities, accelerations, n_at, delta_t, cutoff):
...
def verlet(positions, old_positions, velocities, accelerations, n_at, delta_t, cutoff): # Here f(t) always
    ...
    return positions, velocities, accelerations # Here f(t+delta t)

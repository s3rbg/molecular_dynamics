...
def create_gui():
    ...
    lec.crea_entrada('Number of cells inside the supercell in one direction', "Number of cells", 10)
    ...
    try:
        ...
        for i in ("Density", "Number of cells", "Temperature", 'Max steps', 'Step'):
            try:
                ...
                raise InputError('Invalid type introduced in "{}". Remember to use "." as separator'.format(i))
    ...
    if potential_type == 'lennard-jones' and algo_ode == 'verlet':
        ...
        data = np.array([density, n_cells, epsilon, temperature, cell_type, potential_type, 
                         algo_ode, tot_t, delt_t, sigma, cutoff_distance, cutoff_list, vels_dist, density_unit, sigma_unit,
    ...
    # Save data into a pandas dataframe structure
